class Rule3BuyPoint:
    ...
    def evaluate(self, row: dict) -> bool:
        ...
        tol = self.cfg["touch_ma10_tolerance"]
        ...
        volume_shrink = True
        if vol_ma5 > 0:
            min_vol_ratio = self.cfg["min_volume_ratio"]
            volume_shrink = volume <= vol_ma5 * min_vol_ratio

        # 条件3：周波幅 <= 4%（窄幅整理）
        week_high = row.get("week_high", close)
        week_low = row.get("week_low", close)
        week_range = (week_high - week_low) / (week_low if week_low > 0 else 1)
        narrow_range = week_range <= self.cfg["narrow_range_ratio"]
