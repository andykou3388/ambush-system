...
def oneclick_init_data(self):
    ...
    try:
        ...
        stock_codes = [
            # 1 - 100
            "0001.HK", "0003.HK", "0005.HK", "0006.HK", "0012.HK",
        ...
        result2 = _calculate_weekly_indicators_impl("TW")
        logger.info(f"✅ 週線技術指標計算完成: {result2}")
        
        # 4. 執行規則引擎分析（直接調用函數）
        logger.info("正在執行規則引擎分析...")
        result3 = _run_weekly_rule_engine_impl("TW")
        logger.info(f"✅ 規則引擎分析完成: {result3}")
        
        # 5. 執行每日分鐘線數據校驗（直接調用函數）
        ...
        return {
            "status": "success",
            "message": "所有任務初始化完成",
            "tasks": ["fundamentals", "indicators", "rule_engine", "minute_validation"]
        }
